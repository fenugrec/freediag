...
#ifndef HAVE_STRCASECMP // strcasecmp is POSIX, but kernel32 provides lstrcmpi which should
			// be equivalent.
#	ifdef WIN32
...
#else
#	define UNUSED(X)                                                                 \
		X // how can we suppress "unused parameter" warnings on other compilers?
#endif            // __GNUC__

// hacks for MS Visual studio / visual C
#ifdef MSVC
typedef SSIZE_T ssize_t; // XXX ssize_t is currently only needed because of
			 // diag_tty_unix.c:diag_tty_{read,write}.
			 // TODO : rework read/write types to use a combination of size_t
			 // and int ?
#	define snprintf                                                                  \
		_snprintf // danger : _snprintf doesn't guarantee zero-termination !?
#	pragma message(                                                                  \
		"Warning: MSVC _sprintf() may be dangerous !\
...
// apparently some (all ?) MSVC compilers don't support per-file defines, so
// CURFILE would be the same for all source files. The disadvantage of __FILE__ is
// that it often (always ?) holds the absolute path of the file, not just the
// filename. For our debugging messages we only care about the filename, hence
// CURFILE.
...
#define DIAG_IOCTL_GET_L2_DATA                                                            \
	0x2023 /* Get the L2 Keybytes etc into                                            \
		* diag_l2_data passed to us                                               \
		*/
#define DIAG_IOCTL_SETSPEED                                                               \
	0x2101 /* Set speed, bits etc. data = (const struct diag_serial_settings *); ret  \
		* 0 if ok Ignored if DIAG_L1_AUTOSPEED or DIAG_L1_NOTTY is set */
#define DIAG_IOCTL_INITBUS                                                                \
	0x2201 /* Initialise the ecu bus, data = (struct diag_l1_initbus_args *)          \
		*                                                                         \
		* Caller must have waited the appropriate time before calling this, since \
		* any bus-idle requirements are specified at the L2 level. Must return as \
		* soon as possible, and restore original port settings (speed, etc). See  \
		* diag_l1.h for initbus args return 0 if ok                               \
		*/
#define DIAG_IOCTL_IFLUSH                                                                 \
	0x2202 /* flush input buffers. No data. Ignored if DIAG_L1_NOTTY is set.          \
 ...
 *
 * data can be freed by caller after the ioctl (L0 will make a copy of the message data as
 * required)
 */
#define DIAG_IOCTL_SETWM 0x2203

/****** debug control ******/
// flag containers : diag_l0_debug, diag_l1_debug diag_l2_debug, diag_l3_debug,
// diag_cli_debug
 ...
 * There's probably no application for 0-length messages, but currently the various
 * functions don't complain when allocating/duplicating empty data.
 */
struct diag_msg {
...
#define DIAG_FMT_FRAMED                                                                   \
	0x02 /* Rcvd data is framed, ie not raw. XXX DEPRECATED ! L0..L2 all do this.*/
	     //	#define	DIAG_FMT_DATAONLY	0x04	/* Rcvd data had L2/L3 headers
	     // removed XXX ALWAYS ! */
...
#define DIAG_MSG_IFLAG_MALLOC                                                             \
	1 /* We malloced; we Free -- this is set when the msg                             \
...
int diag_fl_alloc(const char *fName, const int line, void **pp, size_t n, size_t s,
		  bool allocIsCalloc);
 ...
 */
#define diag_calloc(P, N)                                                                 \
	diag_fl_alloc(CURFILE, __LINE__, ((void **)(P)), (N), sizeof(**(P)), true)

/** malloc() with logging.
 * Same as diag_calloc, but without clearing.
 */
#define diag_malloc(P, N)                                                                 \
	diag_fl_alloc(CURFILE, __LINE__, ((void **)(P)), (N), sizeof(**(P)), false)
