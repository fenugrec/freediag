...
#define ELM_INITDONE                                                                      \
	0x10 // set when "BUS INIT" has happened. This is important for
	     // clones.

// possible error messages returned by the ELM IC
static const char *elm323_errors[] = {"BUS BUSY", "FB ERROR", "DATA ERROR", "<DATA ERROR",
				      "NO DATA",  "?",        NULL};

static const char *elm327_errors[] = {
	"BUS BUSY", "FB ERROR",  "DATA ERROR",  "<DATA ERROR", "NO DATA",
	"?",        "ACT ALERT", "BUFFER FULL", "BUS ERROR",   "CAN ERROR",
	"LP ALERT", "LV RESET",  "<RX ERROR",   "STOPPED",     "UNABLE TO CONNECT",
	"ERR",      NULL};

// authentic VS clone identification strings.
// I know of no elm323 clones. 327 clones may not support some commands (atfi, atsi, atkw)
// and thus need fallback methods
static const char *elm323_official[] = {"2.0", NULL}; // authentic 323 firmware versions,
						      // possibly incomplete list
static const char *elm323_clones[] = {NULL};          // known cloned versions
static const char *elm327_official[] = {"1.0a", "1.0", "1.1",  "1.2a", "1.2",
					"1.3a", "1.3", "1.4b", "2.0",  NULL};
static const char *elm327_clones[] = {"1.4", "1.4a", "1.5a", "1.5", "2.1", NULL};

// baud rates for host to elm32x communication. Start with user-specified speed, then try
// common values
...
static int elm_send(struct diag_l0_device *dl0d, UNUSED(const char *subinterface),
		    const void *data, size_t len);

static int elm_sendcmd(struct diag_l0_device *dl0d, const uint8_t *data, size_t len,
		       unsigned int timeout, uint8_t *resp);
...
int
elm_new(struct diag_l0_device *dl0d) {
...
struct cfgi *
elm_getcfg(struct diag_l0_device *dl0d) {
	struct elm_device *dev;
	if (dl0d == NULL) {
...
void
elm_del(struct diag_l0_device *dl0d) {
	struct elm_device *dev;
...
static const char *
elm_parse_errors(struct diag_l0_device *dl0d, uint8_t *data) {
	struct elm_device *dev;
...
// Send a command to ELM device and make sure no error occured. Data is passed on directly
// as a string; caller must make sure the string is \r-terminated , i.e. 0x0D-terminated.
// Sending 0A after 0D will cause ELM to interrupt what it's doing to "process" 0A,
// resulting in a failure. This func should not be used for commands that elicit a data
// response (i.e. all data destined to the OBD bus, hence not prefixed by "AT"). Response
// is dumped in *resp (0-terminated) for optional analysis by caller; *resp must be
// ELM_BUFSIZE long. returns 0 if (prompt_good) && ("OK" found anywhere in the response) &&
// (no known
// error message was present) || 		(prompt_good && ATZ or ATKW command was
// sent) , since response doesn't contain "OK" for ATZ or ATKW. elm_sendcmd should not be
// called from outside diag_l0_elm.c.
static int
elm_sendcmd(struct diag_l0_device *dl0d, const uint8_t *data, size_t len,
	    unsigned int timeout, uint8_t *resp) {
	// note : we better not request (len == (size_t) -1) bytes ! The casts between
	// ssize_t and size_t are
	// "muddy" in here
	// ssize_t xferd;
	int rv;
	...
	if (data[len - 1] != 0x0D) {
		...
		fprintf(stderr, FLFMT "elm_sendcmd: non-terminated command : %.*s\n", FL,
			(int)len, (char *)data);
		// the %.*s is pure magic : limits the string length to len, even if the
		// string is not null-terminated.
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
	diag_tty_iflush(dev->tty_int); // currently the code often "forgets" data in the
				       // input buffer, especially if the previous
	...
	// There is some ambiguity in the ELM datasheets on the exact format of the
	// replies. 1) the prompt character '>' is always alone on its line; 2) depending
	// on some parameters, it may be preceded by 0D or 0D 0A 3) some errors ( "<DATA
	// ERROR" and "<RX ERROR" ) can be appended after a reply; others should be at the
	// beginning of the response (alone on a line)
...
elm_open(struct diag_l0_device *dl0d, int iProtocol) {
	...
	const char **elm_clones; // point to elm323_ or elm327_ clone and official version
				 // string lists
	...
	for (i = 0; elm_speeds[i]; i++) {
		...
		// need to clear that before sending the real ATZ ==> ATI is quick and
		// safe; elm_purge does this.
	...
	if (elm_speeds[i] == 0) {
		fprintf(stderr,
			FLFMT "No response from ELM323/ELM327. Verify connection to ELM\n",
			FL);
		elm_close(dl0d);
	...
	if (strstr((char *)rxbuf, "ELM323") != NULL) {
		...
		fprintf(stderr,
			FLFMT "no valid version string !! Report this !. Got:\n%s\n", FL,
			rxbuf);
	...
	if (rv == 0) {
		...
		dev->elmflags |= ELM_32x_CLONE;
		printf("ELM version not recognized! Please report this ! Response "
		       "was:\n%s\n",
		       rxbuf);
	}
	...
	// if 327, set proto when possible; we won't if 14230, because init type (fast vs
	// slow) isn't decided yet
...
/*
 * Fastinit & slowinit:
 * ELM should manage slow/fast init automatically... Until the code from levels L1 and up
 * can deal with this, the bus will manually be initialized. Only callable internally. Not
 * for export !! Unsupported by some (all?) clones !! (they handle the init internally, on
 * demand)
 */
static int
...
// Only used to force clones to establish a connection... hack-grade because it assumes all
// ECUs support this.
// non-OBD ECUs may not work with this. ELM clones suck...
// TODO : add argument to allow either a 01 00 request (SID1 PID0, J1979) or 3E (iso14230
// TesterPresent) request to force init.
static int
...
elm_updatewm(struct diag_l0_device *dl0d) {
	...
	dev = (struct elm_device *)dl0d->l0_int;
	sprintf((char *)wm, "ATWM %02X %02X %02X %02X\x0D", dev->wm->data[0],
		dev->wm->data[1], dev->wm->data[2], dev->wm->data[3]);
...
elm_initbus(struct diag_l0_device *dl0d, struct diag_l1_initbus_args *in) {
	...
	if (dev->elmflags & ELM_32x_CLONE) {
		printf("Note : explicit bus init not available on clones. Errors here are "
		       "ignored.\n");
	...
	case DIAG_L1_INITBUS_FAST: {
		...
		// explicit init is not supported by clones, they wait for the first OBD
		// request...
	...
	case DIAG_L1_INITBUS_5BAUD:
		...
		if ((dev->elmflags & ELM_323_BASIC) && (in->addr != 0x33)) {
			fprintf(stderr,
				FLFMT
				"elm_initbus: ELM323 doesn't support target address %02X",
				FL, in->addr);
			return diag_iseterr(DIAG_ERR_GENERAL);
		}
		if (in->addr != 0x33) {
		...
		if (dev->elmflags & ELM_INITDONE) {
			...
			if (rv < 0) {
				fprintf(stderr,
					FLFMT
					"elm_initbus: ATKW failed, continuing anyway\n",
					FL);
				...
				rv = 0;
			} else if (sscanf((char *)rxbuf, "1:%02X 2:%02X", &kb1, &kb2) ==
				   2) {
				dev->kb1 = kb1;
				dev->kb2 = kb2;
				in->kb1 = kb1;
		...
		if (dev->elmflags & ELM_INITDONE) {
			...
			if (rv < 0) {
				fprintf(stderr,
					FLFMT
					"elm_initbus: ATAL failed, continuing anyway\n",
					FL);
				rv = 0;
			}
		}
	...
	if ((dev->elmflags & ELM_INITDONE) == 0) {
		// init not done, either because it's a clone (explicit init unsupported),
		// or another reason. two choices : A) assume comms will be OBD compliant,
		// so request "01 00" has to be supported B) tweak _recv() to skip the "BUS
		// INIT: " line that ELM will send at the next request. A is easier for
		// now... Note that since we don't check for "DIAG_ERR_INIT_NOTSUPP" here,
		// we allow the ELM to (possibly) carry out an incorrect init. I can't see
		// when this can be a problem. Note : clones suck
...
static int
elm_purge(struct diag_l0_device *dl0d) {
 ...
 * Upper levels don't append 0x0D / 0x0A at the end, we take care of adding the required
 * 0x0D. "AT"* commands sould not be sent with this function. Returns 0 on success
 */

static int
elm_send(struct diag_l0_device *dl0d, UNUSED(const char *subinterface), const void *data,
	 size_t len) {
	...
	if ((2 * len) > (ELM_BUFSIZE - 1)) {
		...
		fprintf(stderr,
			FLFMT "ELM: too much data for buffer (report this bug please!)\n",
			FL);
		return diag_iseterr(DIAG_ERR_BADLEN);
	}
	...
	for (i = 0; i < len; i++) {
		...
		snprintf((char *)&buf[2 * i], 3, "%02X",
			 (unsigned int)((uint8_t *)data)[i]);
	}
	i = 2 * len;
	buf[i] = 0x0D;
 ...
 * ELM returns a string with format "%02X %02X %02X[...]\n" . But it's slow so we add
 *ELM_SLOWNESS ms to the specified timeout. We convert this received ascii string to hex
 *before returning. note : "len" is the number of bytes read on the OBD bus, *NOT* the
 *number of ASCII chars received on the serial link !
 ...
 *	-still missing : if caller requests 1 byte, and the error is "FB ERROR", we'll
 *still happily return 0xFB !
 *
 * TODO: improve "len" semantics for L0 interfaces that do framing, such as this. Currently
 *this returns max 1 message, to let L2 do another call to get further messages (typical
 *case of multiple responses)
 */
static int
elm_recv(struct diag_l0_device *dl0d, UNUSED(const char *subinterface), void *data,
	 size_t len, unsigned int timeout) {
	int rv, xferd;
	struct elm_device *dev = dl0d->l0_int;
	uint8_t rxbuf[3 * MAXRBUF + 1]; // I think some hotdog code in L2/L3 calls _recv
					// with MAXRBUF so this needs to be huge. the +1 is
					// to \0-terminate the buffer for
					// elm_parse_errors() to work
	...
	while (1) {
		...
		if (sscanf((char *)(&rxbuf[rp]), "%02X", &rbyte) == 1) {
			...
			rv = diag_tty_read(dev->tty_int, rxbuf + wp, sizeof(rxbuf) - wp,
					   ELM_SLOWNESS);
			if (rv >= 0) {
				rxbuf[wp + rv] = 0x00;
			}
		...
		/* we can't read 2 more, in case we're in a multi-message, for instance if
		 * we just decoded 0x00 in "48 6A 01 00\n48 6A..." , reading two bytes
...
static int
elm_setwm(struct diag_l0_device *dl0d, struct diag_msg *pmsg) {
	struct elm_device *dev;
	...
	if (dev->elmflags & ELM_INITDONE) {
		 ...
		 */
		fprintf(stderr,
			FLFMT "elm_setwm: tried to set wakeup message after init\n", FL);
		return diag_iseterr(DIAG_ERR_GENERAL);
	}

	if (dev->elmflags & ELM_323_BASIC) {
		fprintf(stderr,
			FLFMT "elm_setwm: ELM323 doesn't support setting wakeup message\n",
			FL);
...
elm_getflags(struct diag_l0_device *dl0d) {
	...
	case DIAG_L1_ISO9141:
		...
		// flags |= DIAG_L1_NOHDRS;	//probably not needed since we send ATH1 on
		// init (enable headers)
		break;
...
static void
elm_parse_cr(uint8_t *data, int len) {
	int i = 0;
	for (; i < len; i++) {
		if (*data == 0x0D) {
			*data = 0x0A;
...
static int
elm_ioctl(struct diag_l0_device *dl0d, unsigned cmd, void *data) {
	int rv = 0;
...
const struct diag_l0 diag_l0_elm = {"Scantool.net ELM32x Chipset Device",
				    "ELM",
				    DIAG_L1_ISO9141 | DIAG_L1_ISO14230 |
					    DIAG_L1_J1850_PWM | DIAG_L1_J1850_VPW |
					    DIAG_L1_CAN,
				    elm_init,
				    elm_new,
				    elm_getcfg,
