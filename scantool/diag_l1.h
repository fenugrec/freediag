...
#define DIAG_L1_HALFDUPLEX                                                                \
	0x10 /* Physical interface is half duplex, need to remove echos */
...
// NOHDRS
// this indicates that L1 already stripped the headers from the frame (ELM default
// behavior) but the l0_elm init code enables headers so this is not useful at the moment.
// If NOHDRS, DIAG_L1_DOESL2FRAME must also be set! (otherwise, no hope of splitting
// messages...)
 ...
 * @return 0 if ok
 */
int diag_l1_send(struct diag_l0_device *, const char *subinterface, const void *data,
		 size_t len, unsigned int p4);

/** Receive data.
 *
 * @return # of bytes read, DIAG_ERR_TIMEOUT or \<0 if failed. DIAG_ERR_TIMEOUT is not a
 * hard failure since a lot of L2 code uses this to detect end of responses
 */
int diag_l1_recv(struct diag_l0_device *, const char *subinterface, void *data, size_t len,
