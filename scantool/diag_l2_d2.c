...
dl2p_d2_send(struct diag_l2_conn *d_l2_conn, struct diag_msg *msg) {
	...
	rv = diag_l1_send(d_l2_conn->diag_link->l2_dl0d, NULL, buf, msg->len + 3,
...
dl2p_d2_recv(struct diag_l2_conn *d_l2_conn, unsigned int timeout,
	...
	rv = diag_l1_recv(d_l2_conn->diag_link->l2_dl0d, NULL, buf, sizeof(buf),
			  timeout + 100);
	if (rv < 0) {
		return rv;
	}
...
dl2p_d2_request(struct diag_l2_conn *d_l2_conn, struct diag_msg *msg, int *errval) {
	int rv;
...
static int
dl2p_d2_startcomms(struct diag_l2_conn *d_l2_conn, flag_type flags, unsigned int bitrate,
	...
	if (!(d_l2_conn->diag_link->l1flags & DIAG_L1_DOESFULLINIT) ||
	    !(d_l2_conn->diag_link->l1flags & DIAG_L1_DOESL2CKSUM)) {
		fprintf(stderr,
			"Can't do D2 over K-line on this L0 interface yet, sorry.\n");
	...
	if (in.kb1 == 0 && in.kb2 == 0) {
		...
		fprintf(stderr,
			FLFMT
			"_startcomms : L0 didn't return keybytes, continuing anyway\n",
			FL);
	} else {
	...
	if ((d_l2_conn->diag_l2_kb1 != 0xd3) || (d_l2_conn->diag_l2_kb2 != 0xb0)) {
		fprintf(stderr,
			FLFMT "_startcomms : wrong keybytes %02X%02X, expecting D3B0\n",
...
dl2p_d2_stopcomms(struct diag_l2_conn *pX) {
	...
	msg.len = 1;
	msg.dest = 0;
	msg.src = 0; /* use default addresses */
	msg.data = data;
	...
	if (rxmsg == NULL || errval) {
		fprintf(stderr,
			"StopDiagnosticSession request failed, waiting for session to "
			"time out.\n");
...
const struct diag_l2_proto diag_l2_proto_d2 = {DIAG_L2_PROT_D2,
					       "D2",
					       DIAG_L2_FLAG_FRAMED |
