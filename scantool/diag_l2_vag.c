...
struct diag_l2_vag {
	...
	struct diag_msg *ecu_id_telegram; // a pointer to store the ECU ID telegram
	...
	unsigned long long msg_finish_time; // a point in time when the last message
					    // finished arriving/departing
};
...
diag_l2_vag_block_recv(struct diag_l2_conn *d_l2_conn, int *errval, int msg_timeout) {
	...
	while (1) {
		if (d_l2_conn->diag_link->l1flags & DIAG_L1_DOESL2FRAME) {
			...
			// bother validating sequence number
			break;
		}
		...
		if (rv < 0) {
			if (rv == DIAG_ERR_TIMEOUT) {
				// a very special case of timeout is when waiting for the
				// first telegram from the ECU; if it occurs, then it means
				// that the ECU either received wrong KB2 complement (which
				// is more probable) or didn't receive the KB2 at all (less
				// probable); since we cannot OR the error code, let's
				// return the more probable one
				if (dp->first_telegram_started == 0) {
					*errval = DIAG_ERR_BADRATE; // wrong KB2 value
								    // indicates baud rate
								    // problems
					return diag_pseterr(*errval);
				}
				// the timeout may occur if the transmitter didn't receive
				// our complement byte or if that byte was incorrect - in
				// such cases it will retry sending the whole message
				// again, but only after 2*T_R8 time units since it sent
				// the previous message byte; we (the receiver) have been
				// waiting already for T_R8 time unit since sending our
				// complement byte, so the transmitter should re-start the
				// message _within_ _our_ another T_R8 time unit (which
				// started later than the receiver's secont T_R8 time
				// unit);
				rv = diag_l1_recv(d_l2_conn->diag_link->l2_dl0d, 0, &byte,
						  1, KWP1281_T_R8);
				if (rv < 0) {
					// if we timed out again, then this means that the
					// communication line has been broken or closed;
					// but with one exception - if we were waiting for
					// the last byte of the message (the ETX byte),
					// then we should assume that the ETX byte might
					// have been sent by the transmitter, but got lost
					// on its way, and we should try to go on as if we
					// have received it
					...
					// the first byte of the re-started message has
					// arrived, so just reset the rxoffset and go on
		...
		if (dp->rxoffset - 1 == dp->rxbuf[0]) {
			...
			if (byte != KWP1281_END_BYTE ||
			    // check also whether the sequence number present in the
			    // message is correct (should be odd and be greater than our
			    // sequence number by 1)
			    ((dp->rxbuf[1] % 2) == 0 || dp->rxbuf[1] != dp->seq_nr + 1)) {
				// arbitrarily set our sequence number -  we could get here
				// because of an incorrect sequence number sent by the ECU
				dp->seq_nr += 2;
				// send a NoAck Retry message by using the sequence number
				// from the ECU's message (we can be sure that the buffer
				// contains the number that ECU sent us, because when we
				// received it, we responded with a complement and ECU
				// didn't complain)
				noack_data[0] = dp->rxbuf[1];
				// we must flag ourselves as master before calling the send
				// function (and the send function will re-set the flag to
				// slave)
				dp->master = 1;
				...
				// NOTE: SAE J2818 says that "The Message number is NOT
				// incremented by the transmitter for a repeated block."
				//      so we should expect that the repeated message will
				//      have the same sequence number and thus we should
				//      decrease our own sequence number to the previous
				//      value also. However, when testing on an ECU
				//      installed in a European VW, this is not the case -
				//      the ECU repeated the message with an increased
				//      sequence number. Can't test this with a VW from
				//      USA, so commenting-out for now. If US VWs indeed
				//      follow the specification strictly, then a possible
				//      solution would be to set/unset this behaviour based
				//      on a flag (eg. L2_vag specific option for
				//      enabling/disabling strict SAE J2818).
				// dp->seq_nr -= 2;
				// prepare for receiving the whole message again
				dp->rxoffset = 0;
				// time elapsed since receiving last message
				unsigned long long elapsed_time =
					diag_os_hrtus(diag_os_gethrt() -
						      dp->msg_finish_time) /
					1000;
				// the timeout values are between messages, so decrease the
				// T_RB_MAX timeout by the time that has elapsed since
				// sending the no-ack message to the ECU
				timeout = elapsed_time < KWP1281_T_RB_MAX
						  ? KWP1281_T_RB_MAX - elapsed_time
						  : 0;
				continue;
			}
			break;
		...
		unsigned long long elapsed_time =
			diag_os_hrtus(diag_os_gethrt() - byte_recv_time) / 1000;
		// give ECU some time before sending the complement byte
		diag_os_millisleep(elapsed_time < KWP1281_T_R6_MIN
					   ? KWP1281_T_R6_MIN - elapsed_time
					   : 0);
	...
	tmsg->dest = tmsg->src = 0; // these are not used by the protocol (no such info in
				    // message blocks)
	tmsg->fmt |= DIAG_FMT_CKSUMMED; // no real checksum, but the inverted bytes thing
...
diag_l2_vag_int_recv(struct diag_l2_conn *d_l2_conn, unsigned int timeout) {
	...
	// to send an ACK message after every part (message) of the telegram, so prepare
	// one
	...
	while (1) {
		...
		struct diag_msg *tmsg =
			diag_l2_vag_block_recv(d_l2_conn, &rv, msg_timeout);
		if (rv < 0) {
			if (d_l2_conn->diag_msg) {
				diag_freemsg(d_l2_conn->diag_msg);
				d_l2_conn->diag_msg = NULL;
		...
		// if this is the first messag sent by the ECU in the current telegram and
		// this is either ACK or NO_ACK, then pass it to the caller - we cannot do
		// anything about NO_ACK if it was caused by something we didn't send; and
		// if ECU responded only with ACK, then the caller should be informed that
		// there is no other response from the ECU (for instance, the keep-alive
		// routine will be interested in an ACK reply)
		...
		// if the ECU has responded with an ACK message to our ACK message, then it
		// means that the telegram has finished with the previous received message
		...
		if (tmsg->type == KWP1281_SID_NO_ACK) {
			...
			if (tmsg->data[0] == dp->seq_nr - 2) {
				...
				// NOTE: SAE J2818 says that "The Message number is NOT
				// incremented by the transmitter for a repeated block."
				//      so we should send the repeated message with the
				//      same sequence number as the original message.
				//      However, when testing this on an ECU installed in a
				//      European VW the ECU didn't accept such a message
				//      (responded with another NO_ACK Retry) and a
				//      repeated message with an incremented sequence
				//      number was accepted. Can't test this with a VW from
				//      USA, so commenting-out for now. If US VWs indeed
				//      follow the specification strictly, then a possible
				//      solution would be to set/unset this behaviour based
				//      on a flag (eg. L2_vag specific option for
		...
		elapsed_time =
			diag_os_hrtus(diag_os_gethrt() - dp->msg_finish_time) / 1000;
		msg_timeout = elapsed_time < KWP1281_T_RB_MAX
				      ? KWP1281_T_RB_MAX - elapsed_time
				      : 0;
	}
...
dl2p_vag_startcomms(struct diag_l2_conn *d_l2_conn, UNUSED(flag_type flags),
	...
	in.addr = target;
	// NOTE: there is no way to pass the timeout value into the init function -
	// KWP1281_T_R1_MAX
	...
	if ((d_l2_conn->diag_link->l1flags & DIAG_L1_DOESSLOWINIT) == 0) {
		...
		rv = diag_l1_send(d_l2_conn->diag_link->l2_dl0d, 0, cbuf, 1,
	...
	if (rv < 0) {
		// if the error was a timeout while waiting for the very first byte of the
		// telegram, then the error will be set to DIAG_ERR_BADRATE, which means
		// that the ECU informs us that we have probably set incorrect baudrate (it
		// must have received incorrect KB2 complement, and it can happen if we are
		// using incorrect baudrate); ECU will re-try sending the synchronization
		// byte in a moment, but since we are using a user-provided baudrate
		// (instead of decoding it from the sync byte), then we cannot do anything
		// about it here; so just report the error and leave
		free(dp);
		d_l2_conn->diag_l2_proto_data = NULL;
		return diag_iseterr(rv);
	}
	...
	// message interval for use by external timeout handler for sending keep-alive
	// messages
...
static int
dl2p_vag_stopcomms(struct diag_l2_conn *d_l2_conn) {
...
dl2p_vag_send(struct diag_l2_conn *d_l2_conn, struct diag_msg *msg) {
	...
	// the length of the block (counter byte, title byte, data bytes and block end
	// byte)
	...
	unsigned long long elapsed_time =
		diag_os_hrtus(diag_os_gethrt() - dp->msg_finish_time) / 1000;
	diag_os_millisleep(
		elapsed_time < KWP1281_T_RB_MIN ? KWP1281_T_RB_MIN - elapsed_time : 0);
	...
	while (1) {
		...
		// have we just written the last byte? if so, then no inverted response
		// will arrive
		if (dp->rxoffset == dp->rxbuf[0]) {
			dp->msg_finish_time = diag_os_gethrt();
			break;
		...
		rv = diag_l1_recv(d_l2_conn->diag_link->l2_dl0d, 0, &recv_byte, 1,
				  KWP1281_T_R8);
		...
		if (rv < 0) {
			// finish communication if exceeded the max number of retries or if
			// some other error than timeout
			if (++retries > KWP1281_TO_RETRIES || rv != DIAG_ERR_TIMEOUT) {
				return diag_iseterr(rv);
			}
			// retry sending the message
			dp->rxoffset = 0;
			// but only after another t_r8 - we must be sure that the receiver
			// times-out so that it will expect a re-started message
			elapsed_time =
				diag_os_hrtus(diag_os_gethrt() - byte_sent_time) / 1000;
			diag_os_millisleep(elapsed_time < 2 * KWP1281_T_R8
						   ? 2 * KWP1281_T_R8 - elapsed_time
						   : 0);
			continue;
		}
		...
		elapsed_time =
			diag_os_hrtus(diag_os_gethrt() - complement_recv_time) / 1000;
...
dl2p_vag_recv(struct diag_l2_conn *d_l2_conn, unsigned int timeout,
	...
	// the message (ECU ID telegram) is already read, so call int_recv() only if the
	// little shiny present has already been collected
	struct diag_l2_vag *dp = (struct diag_l2_vag *)d_l2_conn->diag_l2_proto_data;
	if (dp->ecu_id_telegram == NULL) {
	...
	// NOTE: if ECU returned NO_ACK, then the caller won't know what type it was (retry
	// or unknown)
	if (callback) {
...
dl2p_vag_request(struct diag_l2_conn *d_l2_conn, struct diag_msg *msg, int *errval) {
	...
	while (1) {
		...
		// re-send with the previous sequence number
		// NOTE: SAE J2818 says that "The Message number is NOT incremented by the
		// transmitter for a repeated block."
		//      so we should send the repeated message with the same sequence
		//      number as the original message. However, when testing this on an
		//      ECU installed in a European VW the ECU didn't accept such a message
		//      (responded with another NO_ACK Retry) and a repeated message with
		//      an incremented sequence number was accepted. Can't test this with a
		//      VW from USA, so commenting-out for now. If US VWs indeed follow the
		//      specification strictly, then a possible solution would be to
		//      set/unset this behaviour based on a flag (eg. L2_vag specific
		//      option for enabling/disabling strict SAE J2818).
		// dp->seq_nr -= 2;
	}
...
const struct diag_l2_proto diag_l2_proto_vag = {
	DIAG_L2_PROT_VAG,   "VAG",         DIAG_L2_FLAG_KEEPALIVE, dl2p_vag_startcomms,
	dl2p_vag_stopcomms, dl2p_vag_send, dl2p_vag_recv,          dl2p_vag_request,
	dl2p_vag_timeout};
