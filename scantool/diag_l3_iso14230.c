 ...
 */
char *
diag_l3_iso14230_decode_response(struct diag_msg *msg, char *buf, const size_t bufsize) {
	char buf2[80];
	...
	default:
		...
		if (msg->data[0] & 0x40) {
			...
			case DIAG_KW2K_SI_REID:
				snprintf(buf2, sizeof(buf2), "identOption 0x%02X",
					 msg->data[1]);
				smartcat(buf, bufsize, buf2);
				break;
			case DIAG_KW2K_SI_RDDBLI:
				snprintf(buf2, sizeof(buf2), "RLOCID 0x%02X",
					 msg->data[1]);
				smartcat(buf, bufsize, buf2);
				break;
			default:
			...
			// or maybe expand the SID tables to include a field that indicates
			// how many extra bytes are meaningful. For example, SID 27
			// SecurityAccess
...
diag_l3_14230_rxcallback(void *handle, struct diag_msg *msg) {
	...
	if (msg->fmt & DIAG_FMT_FRAMED) {
		...
		fprintf(stderr,
			FLFMT
			"problem: got an unframed message!\n"
			"Report this !\n",
			FL);
	}
}
...
// iso14230 L3 over anything else than an iso14230 L2, so there's no reason to do anything
// else in here.
static int
diag_l3_iso14230_recv(struct diag_l3_conn *d_l3_conn, unsigned int timeout,
	...
	if (d_l3_conn->d_l3l2_flags & DIAG_L2_FLAG_FRAMED) {
		...
		fprintf(stderr,
			FLFMT "*** Error : using iso14230 L3 code on a non-iso14230\n",
			FL);
...
void
diag_l3_iso14230_decode(UNUSED(struct diag_l3_conn *d_l3_conn), struct diag_msg *msg,
...
static const char *
l3_iso14230_sidlookup(const int id) {
	unsigned i;
...
static const char *
l3_iso14230_neglookup(const int id) {
	unsigned i;
