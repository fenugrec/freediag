 ...
 * Right now there's no self-checking but it should be of OK accuracy for basic stuff (
 keepalive messages )
...
LARGE_INTEGER perfo_freq = {{0, 0}}; // for use with QueryPerformanceFrequency and
				     // QueryPerformanceCounter
float pf_conv = 0; // this will be (1E6 / perfo_freq) to convert counts to microseconds,
		   // i.e. [us]=[counts]*pf_conv
static int pfconv_valid = 0; // flag after querying perfo_freq; nothing will not work
...
VOID CALLBACK
timercallback(UNUSED(PVOID lpParam), BOOLEAN timedout) {
...
diag_os_init(void) {
	...
	// probably the nearest equivalent to a unix interval timer + associated alarm
	// handler is the timer queue... so that's what we do. we create the timer in the
	// default timerqueue
	InitializeCriticalSection(&periodic_lock);

	if (!CreateTimerQueueTimer(&hDiagTimer, NULL, (WAITORTIMERCALLBACK)timercallback,
				   NULL, tmo, tmo, WT_EXECUTEDEFAULT)) {
	...
	// From MSDN docs :	The frequency of the performance counter is fixed at system
	// boot 					and is consistent across all
	// processors.
	// Therefore, the frequency 					need only
	// be queried upon application initialization, and the result can be cached.
	//
	//					Under what circumstances does
	// QueryPerformanceFrequency return FALSE, 					or
	// QueryPerformanceCounter return zero?
	//						->This won't occur on any system
	// that runs Windows XP or later.
...
diag_os_sched(void) {
	...
	// the resultant "base priority" is a combination of process priority and thread
	// priority.
	...
	if (!SetPriorityClass(curprocess, HIGH_PRIORITY_CLASS)) {
		fprintf(stderr,
			FLFMT
			"Warning: could not increase process priority. Timing may be "
			"impaired.\n",
			FL);
	}
	...
	if (!SetThreadPriority(curthread, THREAD_PRIORITY_HIGHEST)) {
		fprintf(stderr,
			FLFMT
			"Warning : could not increase thread priority. Timing may be "
			"impaired.\n",
			FL);
	}
	rv = 0;
...
//
const char *
diag_os_geterr(OS_ERRTYPE os_errno) {
	...
	if (os_errno != 0) {
		DWORD elen = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
						   FORMAT_MESSAGE_FROM_SYSTEM,
					   NULL, os_errno, 0, (LPTSTR)&errstr, 0, NULL);
		if (elen) {
...
#define RESOL_ITERS 10
	...
	printf("diag_os_gethrt() resolution <= %luus, avg ~%luus\n",
	       (unsigned long)diag_os_hrtus(maxres),
	       (unsigned long)diag_os_hrtus(resol / RESOL_ITERS));
...
#define CAL_ITERS 6
		...
		avgerr = (LONGLONG)(((tsum / CAL_ITERS) - counts) * pf_conv); // average
									      // error in
									      // us
		...
		if ((min < counts) || (avgerr > 900))
			printf("diag_os_millisleep(%d) off by %+" PRId64 "%% (%+" PRId64
			       "us)"
			       "; spread=%" PRIu64 "%%\n",
			       testval, (avgerr * 100 / 1000) / testval, avgerr,
			       ((max - min) * 100) / counts);
...
unsigned long long
diag_os_hrtus(unsigned long long hrdelta) {
	assert(pfconv_valid);
