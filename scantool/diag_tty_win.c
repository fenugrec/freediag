...
diag_tty_open(const char *portname) {
	...
	// Now, in case of errors we can call diag_tty_close() on wti since its members are
	// alloc'ed
	...
	if (wti->fd != INVALID_HANDLE_VALUE) {
		...
		fprintf(stderr, FLFMT "Open of device interface \"%s\" failed: %s\n", FL,
			wti->name, diag_os_geterr(0));
		fprintf(stderr,
			FLFMT "(Make sure the device specified corresponds to the\n", FL);
		fprintf(stderr, FLFMT "serial device your interface is connected to.\n",
			FL);
	...
	// as opposed to the unix diag_tty.c ; this one doesn't save previous commstate.
	// The next program to use the COM port will need to deal with it...

	// We will load the DCB with the current comm state. This way we only need to call
	// GetCommState once during a session and the DCB should contain coherent initial
	// values
	...
	devtimeouts.ReadTotalTimeoutConstant = 20;  // (constant + multiplier*numbytes) =
						    // total timeout on read(buf, numbytes)
	devtimeouts.WriteTotalTimeoutMultiplier = 0; // probably useless as all flow
 ...
 */
int
diag_tty_setup(ttyp *ttyh, const struct diag_serial_settings *pset) {
	...
	// optional : verify if device supports the requested settings. Testing required to
	// see if USB->serial bridges support this ! i.e. some l0 devices try to set 5bps
	// which isn't supported on some devices. For now let's just check if it supports
	// custom baud rates. This check should be added to diag_tty_open where it would
	// set appropriate flags to allow _l0 devices to adapt their functionality.
	if (!GetCommProperties(devhandle, &supportedprops)) {
		fprintf(stderr, FLFMT "could not getcommproperties: %s\n", FL,
			diag_os_geterr(0));
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
	// simple test : only check if custom baud rates are supported, but don't abort if
	// they aren't. Just notify user.
	 ...
	 */
	// the DCB (devstate) has been loaded with initial values in diag_tty_open so it
	// should already coherent.
	...
	case diag_par_o:
		...
		break;
	default:
		fprintf(stderr, FLFMT "bad parity setting used !\n", FL);
		return diag_iseterr(DIAG_ERR_GENERAL);
		break;
	}
	devstate->fOutxCtsFlow = 0;
	devstate->fOutxDsrFlow = 0;                  // disable output flow control
	devstate->fDtrControl = DTR_CONTROL_DISABLE; // XXX allows to permanently set the
						     // DTR line !
	...
	// to be really thorough we do another GetCommState and check that the speed was
	// set properly. This *may* help to detect if the serial port supports non-standard
	// baudrates (5bps being particularly problematic with USB->serial converters) I
	// see no particular reason to check all the other fields though.
	...
	if (verif_dcb.BaudRate != pset->speed) {
		fprintf(stderr, FLFMT "SetCommState failed : speed is currently %u\n", FL,
			(unsigned int)verif_dcb.BaudRate);
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
 ...
 * If it proves to be a problem, it's possible to change both RTS and DTR at once by
 * updating the DCB and calling SetCommState. That call would take around 30-40us. Note :
 * passing 1 in dtr or rts means "set DTR/RTS", i.e. positive voltage. ret 0 if ok
 */
int
...
ssize_t
diag_tty_write(ttyp *ttyh, const void *buf, const size_t count) {
	DWORD byteswritten;
	OVERLAPPED *pOverlap;
	struct tty_int *wti = ttyh;
	pOverlap = NULL; // note : if overlap is eventually enabled, the CreateFile flags
			 // should be adjusted
	...
	if (!WriteFile(wti->fd, buf, count, &byteswritten, pOverlap)) {
		fprintf(stderr,
			FLFMT "WriteFile error:%s. %u bytes written, %u requested\n", FL,
			diag_os_geterr(0), (unsigned int)byteswritten, (unsigned)count);
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
...
diag_tty_read(ttyp *ttyh, void *buf, size_t count, unsigned int timeout) {
	...
	// and modify them
	devtimeouts.ReadIntervalTimeout = timeout ? 0 : MAXDWORD; // disabled unless
								  // timeout was 0.
	devtimeouts.ReadTotalTimeoutMultiplier = 0;     // timeout per requested byte
	devtimeouts.ReadTotalTimeoutConstant = timeout; // (tconst + mult*numbytes) = total
							// timeout on read
	devtimeouts.WriteTotalTimeoutMultiplier = 0;    // probably useless as all flow
							// control will be disabled ??
	devtimeouts.WriteTotalTimeoutConstant = 0;
...
// ret 0 if ok
int
diag_tty_break(ttyp *ttyh, const unsigned int ms) {
	...
	// that call can take quite a while (6ms !!) on some setups (win7 + CH340
	// USB-Serial). It's still impossible to know (from here) when exactly TXD goes low
	// (beginning or end of the call)
...
/*
 * diag_tty_fastbreak: send 0x00 at 360bps => fixed 25ms break; return [ms] after starting
 * break. This is for ISO14230 fast init : typically diag_tty_fastbreak(tty_int, 50) It
 * assumes the interface is half-duplex. Ret 0 if ok
 */
int
diag_tty_fastbreak(ttyp *ttyh, const unsigned int ms) {
	...
	tremain = ((LONGLONG)tremain * 1000) / perfo_freq.QuadPart; // convert to ms;
								    // imprecise but that
								    // should be OK.
...
diag_tty_getportlist(int *numports) {
	...
	char ValueName[256] = "";
	unsigned long szValueName = 256; // variable that specifies the size (in
					 // characters, including the terminating null
					 // char) of the buffer pointed to by the
					 // "ValueName" parameter.
	unsigned char Data[256] = ""; // buffer that receives the data for the value entry.
				      // This parameter can be NULL if the data is not
				      // required
	unsigned long szData = 256;   // variable that specifies the size, in bytes, of the
	...
	*numports = 0;
	...
	cv = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "HARDWARE\\DEVICEMAP\\SERIALCOMM", 0,
			   KEY_READ, &hKey);
	if (cv == ERROR_SUCCESS) {
		while ((RegEnumValueA(hKey, index, ValueName, &szValueName, NULL, NULL,
				      Data, &szData)) == ERROR_SUCCESS) {
			if (!strncmp((char *)Data, "COM", 3)) {
				// CHECK IF PORT IS AVAILABLE (not in use):
				char NTdevName[30] = "\\\\.\\"; // => "\\.\"
				strncpy(NTdevName + 4, (char *)Data, 25);
				/* NOTE: MS-DOS device names ("COMx") are not reliable if x
				   is > 9 !!!
					=> device can not be opened (error 2 "The system
				   cannot find the file specified.") Using NT device names
				   instead ("\\.\COMx") which work in all cases.
				*/
				hCom_t = CreateFileA(
					NTdevName, // device name of the port
				...
				if (hCom_t != INVALID_HANDLE_VALUE) {
					CloseHandle(hCom_t);
					char **templist =
						strlist_add(portlist, NTdevName, elems);
					if (!templist) {
		...
		// std::sort(portlist.begin(), portlist.end());	// quicksort from
		// <algorithm>
