...
static int
dyno_check_allocated_table(struct dyno_measure_table *table) {
	int rv;
	if (table->meas.measures == NULL) {
		...
		memcpy(ptr, table->meas.measures,
		       table->nbr * sizeof(*(table->meas.measures)));
...
/* reset table content */
static int
dyno_table_reset(struct dyno_measure_table *table) {
	if (table->meas.measures != NULL) {
		free(table->meas.measures);
 ...
 */
int
dyno_set_gear(int speed, int rpm) {
 ...
 */
int
dyno_get_speed_from_rpm(int rpm) {
	int speed;
...
/* Add loss measure */
int
dyno_loss_add_measure(int millis, int speed) {
...
dyno_loss_a_inter(int i, int j) {
	...
	/* get acceleration */
	a = 1.0 * (measure1->speed - measure0->speed) /
...
void
dyno_loss_set_d(double d) {
	dyno_loss_d = d;
}

void
dyno_loss_set_f(double f) {
	dyno_loss_f = f;
}

/* Calculate loss power from speed (m/s2 * 1000) */
static long
dyno_loss_power(long speed) {
	double power;
...
/* Add a measure */
int
dyno_add_measure(int millis, int rpm) {
	/* check memory allocation */
...
int
dyno_get_nb_measures() {
	return measure_table.nbr;
}

/* Get all measures */
int
dyno_get_measures(dyno_measure *measures, int size) {
	/* copy measures */
	memcpy(measures, measure_table.meas.measures,
	       MIN(size, measure_table.nbr) * sizeof(dyno_measure));
 ...
 */
static void
dyno_calculate_result(dyno_measure *measure0, dyno_measure *measure1,
		      dyno_result *result) {
 ...
 */
static void
dyno_calculate_results(dyno_result *results) {
	int i;
	int nb = dyno_get_nb_results();

	/* for each measure, calculate result */
	for (i = 0; i < nb; i++) {
		dyno_calculate_result(&measure_table.meas.measures[i],
				      &measure_table.meas.measures[i + 1], &results[i]);
	}
}
...
int
dyno_get_results(dyno_result *results, UNUSED(int size)) {
 ...
 */
int
dyno_smooth_results(dyno_result *results, int size) {
	...
	for (i = 1; i < size - 1; i++) {
		...
		results[i].power = (rawresults[i - 1].power + rawresults[i].power +
				    rawresults[i + 1].power) /
				   3;
 ...
 */
void
dyno_save(char *filename, dyno_result *results, int size) {
	char buffer[MAXRBUF];
	int i;
	...
	for (i = 0; i < size; i++) {
		sprintf(buffer, "%d\t%d\t%d\t%d\n", results[i].rpm, results[i].power,
			results[i].power_ch, results[i].torque);
		fwrite(buffer, strlen(buffer), sizeof(char), fd);
	}
	...
	if (loss_measure_table.nbr > 0) {
		...
		for (i = 0; i < loss_measure_table.nbr; i++) {
			sprintf(buffer, "%d\t%7.3f\t%7.3f\n",
				...
				loss_measure_table.meas.loss_measures[i].speed * 3.6 /
					1000.0);
			fwrite(buffer, strlen(buffer), sizeof(char), fd);
		}
	}
