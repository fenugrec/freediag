	...
	LL_FOREACH(msg, tmsg) {
			...
			LL_FOREACH(msg, tmsg) {
				...
				if ((data[2] & 0x80) == 0) {
					if (ihandle == RQST_HANDLE_NCMS2) {
						...
						if (val > lim) {
							fprintf(stderr,
								"Test 0x%X Component 0x%X "
								"FAILED ",
								data[1], data[2] & 0x7f);
							fprintf(stderr,
								"Max val %d Current Val "
								"%d\n",
								lim, val);
						}
					} else {
		...
		case RQST_HANDLE_O2S:
			...
			if ((data[1] & 0x1f) == 0) {
				...
				case 9:
					fprintf(stderr,
						"%s: actual %2.2f min %2.2f max %2.2f "
						"%s\n",
				...
				case 8:
					fprintf(stderr, "%s: %f %f %f %s\n",
						O2_strings[data[1]], data[4] / 200.,
						data[5] / 200., data[6] / 200.,
						failed ? "FAILED" : "Passed");
					break;
				default:
					fprintf(stderr,
						"Test %d: actual 0x%X min 0x%X max 0x%X "
						"%s\n",
...
j1979_watch_rcv(void *handle, struct diag_msg *msg) {
	...
	LL_FOREACH(msg, tmsg) {
		...
		if (handle != NULL) {
			...
			diag_l3_decode((struct diag_l3_conn *)handle, tmsg, buf,
				       sizeof(buf));
			fprintf(stderr, "%s\n", buf);
		} else {
...
#define J1979_MODE_MAX 9
	...
	if (rv < 0) {
		...
		if (rv < 0) {
			...
			rv = d_conn->d_l3_proto->diag_l3_proto_timer(d_conn,
								     6000); // force
									    // keepalive
			if (rv < 0) {
				fprintf(stderr,
					"\tfailed, connection to ECU may be lost!\n");
				return diag_iseterr(rv);
			}
			fprintf(stderr, "\tOK.\n");
	...
	// This part is super confusing: ihandle comes from the handle from a callback
	// passed between L2 and L3 with handles to handles etc..
	switch (ihandle) {
	...
	for (i = 0, ep = ecu_info; i < ecu_count; i++, ep++) {
		if (ep->rxmsg) {
			...
			/* A bit of ugliness is required to bail out on NegativeResponse
			 * messages when using an iso14230 L2.
			 */
			if (d_conn->d_l3l2_conn->l2proto->diag_l2_protocol ==
			...
			case 1:
				...
				memcpy(ep->mode1_data[p1].data, rxdata, rxmsg->len);
			...
			case 2:
				...
				memcpy(ep->mode2_data[p1].data, rxdata, rxmsg->len);
 ...
 */
static struct diag_l2_conn *
do_l2_common_start(int L1protocol, int L2protocol, flag_type type, unsigned int bitrate,
	...
	d_conn = diag_l2_StartCommunications(dl0d, L2protocol, type, bitrate, target,
					     source);
...
do_l2_9141_start(int destaddr) {
	...
	d_conn = do_l2_common_start(DIAG_L1_ISO9141, DIAG_L2_PROT_ISO9141,
				    ...
				    (uint8_t)destaddr, global_cfg.src);
...
do_l2_14230_start(int init_type) {
	...
	d_conn = do_l2_common_start(DIAG_L1_ISO14230, DIAG_L2_PROT_ISO14230, flags,
...
do_l2_j1850_start(int l1_type) {
	...
	d_conn = do_l2_common_start(l1_type, DIAG_L2_PROT_SAEJ1850, flags,
				    global_cfg.speed, 0x6a, global_cfg.src);
 ...
 * becoming ready for read (using diag_os_ipending()), which amounts to "was Enter
 * pressed".
 *
...
do_j1979_getdata(int interruptible) {
	...
	diag_os_ipending(); // this is necessary on WIN32 to "purge" the last state of the
			    // enter key; we can't just poll stdin.
	...
	for (i = 3; i < 0x100; i++) {
		if (merged_mode1_info[i]) {
			...
			rv = l3_do_j1979_rqst(d_conn, 0x1, (uint8_t)i, 0x00, 0x00, 0x00,
					      0x00, 0x00, (void *)&_RQST_HANDLE_NORMAL);
			if (rv < 0) {
	...
	rv = l3_do_j1979_rqst(d_conn, 0x2, 2, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	for (j = 0, ep = ecu_info; j < ecu_count; j++, ep++) {
		if ((ep->mode2_data[2].type == TYPE_GOOD) &&
		    (ep->mode2_data[2].data[2] | ep->mode2_data[2].data[3])) {
			for (i = 3; i < 0x100; i++) {
				if (ep->mode2_info[i]) {
					fprintf(stderr,
						"Requesting Mode 0x02 Pid 0x%02X...\n", i);
					rv = l3_do_j1979_rqst(
						d_conn, 0x2, (uint8_t)i, 0x00, 0x00, 0x00,
						0x00, 0x00, (void *)&_RQST_HANDLE_NORMAL);
					if (rv < 0) {
						fprintf(stderr,
							"Mode 0x02 Pid 0x%02X request "
							"failed (%d)\n",
							i, rv);
					} else {
						msg = find_ecu_msg(0, 0x42);
	...
	for (i = 0, ep = ecu_info; i < ecu_count; i++, ep++) {
		if ((ep->mode1_data[2].type == TYPE_GOOD) &&
			...
			fprintf(stderr, "ECU %d Freezeframe data exists, caused by DTC ",
				i);
			print_single_dtc(ep->mode1_data[2].data[2],
...
print_single_dtc(databyte_type d0, databyte_type d1) {
	...
	fprintf(stderr, "%s",
		diag_dtc_decode(db, 2, NULL, NULL, dtc_proto_j2012, buf, sizeof(buf)));
...
do_j1979_cms() {
	...
	if (rv != 0) {
		fprintf(stderr,
			"Failed to get test results for continuously monitored systems\n");
		return;
	}
...
do_j1979_ncms(int printall) {
	...
	for (i = 0; i < 60; i++) {
		if ((merged_mode6_info[i]) && ((i & 0x1f) != 0)) {
			...
			rv = l3_do_j1979_rqst(d_conn, 6, (uint8_t)i, 0x00, 0x00, 0x00,
					      0x00, 0x00,
					      (void *)(printall ? &_RQST_HANDLE_NCMS
								: &_RQST_HANDLE_NCMS2));
			if (rv < 0) {
...
/*
 * Get mode info
 * response_offset : index into received packet where the the supported_pid bytemasks
 * start.
 * TODO: add return value to signal total failure (if l3_do_j1979_rqst lost the connection
 * to the ECU)
 */
void
do_j1979_getmodeinfo(uint8_t mode, int response_offset) {
	...
	for (pid = 0; pid < 0x100; pid += 0x20) {
		...
		fprintf(stderr, "Exploring Mode 0x%02X supported PIDs (block 0x%02X)...\n",
			mode, pid);
		rv = l3_do_j1979_rqst(d_conn, mode, (uint8_t)pid, 0x00, 0x00, 0x00, 0x00,
				      0x00, (void *)&_RQST_HANDLE_NORMAL);
		if (rv != 0) {
			/* No response */
			break;
		}
		...
		for (j = 0, ep = ecu_info, not_done = 0; j < ecu_count; j++, ep++) {
			...
			for (i = 0; i <= 0x20; i++) {
				if (l2_check_pid_bits(&ep->rxmsg->data[response_offset],
						      (int)i)) {
					data[i + pid] = 1;
				}
			}
...
do_j1979_getO2tests(int O2sensor) {
	...
	for (i = 1; i <= 0x1f; i++) {
		...
		if ((merged_mode5_info[i]) && ((i & 0x1f) != 0)) {
			...
			rv = l3_do_j1979_rqst(d_conn, 5, (uint8_t)i, o2s, 0x00, 0x00, 0x00,
					      0x00, (void *)&_RQST_HANDLE_O2S);
...
do_j1979_getdtcs() {
	...
	if (merged_mode1_info[1] == 0) {
		fprintf(stderr,
			"ECU(s) do not support DTC#/test query - can't do "
			"tests\n");
		return 0;
	}

	fprintf(stderr, "Requesting Mode 0x01 PID 0x01 (Current DTCs)...\n");
	rv = l3_do_j1979_rqst(d_conn, 1, 1, 0, 0x00, 0x00, 0x00, 0x00,
			      (void *)&_RQST_HANDLE_NORMAL);
	...
	if (num_dtcs) {
		...
		rv = l3_do_j1979_rqst(d_conn, 3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				      (void *)&_RQST_HANDLE_NORMAL);
...
do_j1979_getO2sensors() {
	...
	rv = l3_do_j1979_rqst(d_conn, 1, 0x13, 0, 0x00, 0x00, 0x00, 0x00,
			      (void *)&_RQST_HANDLE_NORMAL);
...
diag_cleardtc(void) {
	...
	rv = l3_do_j1979_rqst(d_conn, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			      (void *)&_RQST_HANDLE_NORMAL);
...
static void
format_o2(char *buf, int maxlen, UNUSED(int english), const struct pid *p, response *data,
	  int n) {
...
static void
format_aux(char *buf, int maxlen, UNUSED(int english), const struct pid *p, response *data,
	   int n) {
...
static void
format_data(char *buf, int maxlen, int english, const struct pid *p, response *data,
	    int n) {
	double v;
...
static const struct pid pids[] = {
	{0x03, "Fuel System Status", format_fuel, 2, "", 0.0, 0.0, "", 0.0, 0.0},
	{0x04, "Calculated Load Value", format_data, 1, "%5.1f%%", (100.0 / 255), 0.0, "",
	 0.0, 0.0},
	{0x05, "Engine Coolant Temperature", format_data, 1, "%3.0fC", 1, -40, "%3.0fF",
	 1.8, 32},
	{0x06, "Short term fuel trim Bank 1", format_data, 1, "%5.1f%%", (100.0 / 128),
	 -100, "", 0.0, 0.0},
	{0x07, "Long term fuel trim Bank 1", format_data, 1, "%5.1f%%", (100.0 / 128),
	 -100, "", 0.0, 0.0},
	{0x08, "Short term fuel trim Bank 2", format_data, 1, "%5.1f%%", (100.0 / 128),
	 -100, "", 0.0, 0.0},
	{0x09, "Long term fuel trim Bank 2", format_data, 1, "%5.1f%%", (100.0 / 128),
	 -100, "", 0.0, 0.0},
	{0x0a, "Fuel Pressure", format_data, 1, "%3.0fkPaG", 3.0, 0.0, "%4.1fpsig",
	 0.14503774, 0.0},
	{0x0b, "Intake Manifold Pressure", format_data, 1, "%3.0fkPaA", 1.0, 0.0,
	 "%4.1finHg", 0.29529983, 0.0},
	{0x0c, "Engine RPM", format_data, 2, "%5.0fRPM", 0.25, 0.0, "", 0.0, 0.0},
	{0x0d, "Vehicle Speed", format_data, 1, "%3.0fkm/h", 1.0, 0.0, "%3.0fmph",
	 0.62137119, 0.0},
	{0x0e, "Ignition timing advance Cyl #1", format_data, 1, "%4.1f deg", 0.5, -64.0,
	 "", 0.0, 0.0},
	{0x0f, "Intake Air Temperature", format_data, 1, "%3.0fC", 1.0, -40.0, "%3.0fF",
	 1.8, 32.0},
	{0x10, "Air Flow Rate", format_data, 2, "%6.2fgm/s", 0.01, 0.0, "%6.1flb/min",
	 0.13227736, 0.0},
	{0x11, "Absolute Throttle Position", format_data, 1, "%5.1f%%", (100.0 / 255), 0.0,
	 "", 0.0, 0.0},
	{0x12, "Commanded Secondary Air Status", format_data, 1, "", 0, 0, "", 0,
	 0}, // can't format bit fields
	{0x13, "Location of Oxygen Sensors", format_data, 1, "", 0, 0, "", 0, 0}, // can't
										  // format
										  // bit
										  // fields
	{0x14, "Bank 1 Sensor 1 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x15, "Bank 1 Sensor 2 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x16, "Bank 1 Sensor 3 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x17, "Bank 1 Sensor 4 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x18, "Bank 2 Sensor 1 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x19, "Bank 2 Sensor 2 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x1a, "Bank 2 Sensor 3 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x1b, "Bank 2 Sensor 4 Voltage/Trim", format_o2, 2, "%5.3fV", 0.005, 0.0,
	 "%5.3fV/%5.1f%%", (100.0 / 128), -100.0},
	{0x1e, "Auxiliary Input Status", format_aux, 1, "", 0.0, 0.0, "", 0.0, 0.0},
};

const struct pid *
get_pid(unsigned int i) {
