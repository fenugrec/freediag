...
const struct cmd_tbl_entry debug_cmd_table[] = {
	{"help", "help [command]", "Gives help for a command", cmd_debug_help, 0, NULL},
	{"?", "? [command]", "Gives help for a command", cmd_debug_help, FLAG_HIDDEN,
	 NULL},

	{"show", "show", "Shows current debug levels", cmd_debug_show, 0, NULL},

	{"l0", "l0 [val]", "Show/set Layer0 debug level", cmd_debug_l0, 0, NULL},
	{"l1", "l1 [val]", "Show/set Layer1 debug level", cmd_debug_l1, 0, NULL},
	{"l2", "l2 [val]", "Show/set Layer2 debug level", cmd_debug_l2, 0, NULL},
	{"l3", "l3 [val]", "Show/set Layer3 debug level", cmd_debug_l3, 0, NULL},
	{"cli", "cli [val]", "Show/set CLI debug level", cmd_debug_cli, 0, NULL},
	{"all", "all [val]", "Show/set All layer debug level", cmd_debug_all, 0, NULL},
	{"l0test", "l0test [testnum]",
	 "Dumb interface tests. Disconnect from vehicle first !", cmd_debug_l0test, 0,
	 NULL},
...
cmd_debug_help(int argc, char **argv) {
	if (argc < 2) {
		printf("Debugging flags are set per level according to the values set in "
		       "diag.h\n");
		printf("Setting [val] to -1 will enable all debug messages for that "
		       "level.\n"
		       "Available flags:\n");
		int i;
...
static int
cmd_debug_l0test(int argc, char **argv) {
#define MAX_L0TEST 14
	struct diag_l0_device *dl0d = global_dl0d;
	unsigned int testnum = 0;

	if ((argc <= 1) || (strcmp(argv[1], "?") == 0) ||
	    (sscanf(argv[1], "%u", &testnum) != 1)) {
		printf("usage: %s [testnum], where testnum is a number between 1 and "
		       "%d.\n",
		       argv[0], MAX_L0TEST);
		printf("you must have done \"set interface dumbt [port]\" and \"set "
		       "dumbopts\" before proceding.\n");

		printf("Available tests:\n"
		       "\t1 : slow pulse TXD (K) with diag_tty_break.\n"
		       "\t2 : fast pulse TXD (K) : send 0x55 @ 10400bps, 5ms interbyte "
		       "(P4)\n"
		       "\t10: fast pulse TXD (K) : send 0x55 @ 15000bps, 5ms interbyte "
		       "(P4)\n"
		       "\t3 : slow pulse RTS.\n"
		       "\t4 : slow pulse DTR.\n"
