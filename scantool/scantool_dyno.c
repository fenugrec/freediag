...
const struct cmd_tbl_entry dyno_cmd_table[] = {
	{"help", "help [command]", "Gives help for a command", cmd_dyno_help, 0, NULL},
	{"?", "? [command]", "Gives help for a command", cmd_dyno_help, FLAG_HIDDEN, NULL},

	{"mass", "mass [mass]", "Step 1 : Shows/Sets the mass of the vehicle",
	 cmd_dyno_mass, 0, NULL},
	{"loss", "loss",
	 "Step 2 : Determines power lost by aerodynamic and friction forces",
	 cmd_dyno_loss, 0, NULL},
	{"setloss", "setloss [d] [f]",
	 "Manually enter aerodynamic and friction forces parameters", cmd_dyno_setloss, 0,
	 NULL},
	{"run", "run", "Step 3 : Run dyno", cmd_dyno_run, 0, NULL},
	{"measures", "measures", "Display run measures", cmd_dyno_measures, 0, NULL},
	{"result", "result", "Display run results", cmd_dyno_result, 0, NULL},
	{"graph", "graph", "Display run graphs", cmd_dyno_graph, 0, NULL},
	{"save", "save [filename]", "Save measures and results in a file", cmd_dyno_save,
	 0, NULL},
 ...
 */
static int
cmd_dyno_mass(int argc, char **argv) {
	int mass = 0;
...
// return <0 if error
static int
measure_data(uint8_t data_pid, ecu_data *ep) {
	int rv;
	...
	/* measure */
	rv = l3_do_j1979_rqst(global_l3_conn, 0x1, data_pid, 0x00, 0x00, 0x00, 0x00, 0x00,
...
fake_loss_measure_data() {
	...
	if (elapsed < 11983)
		speed = 89 * 10000 / 36;
	else if (elapsed < 12723)
		speed = 88 * 10000 / 36;
	else if (elapsed < 13473)
		speed = 87 * 10000 / 36;
	else if (elapsed < 14713)
		speed = 85 * 10000 / 36;
	else if (elapsed < 15713)
		speed = 84 * 10000 / 36;
	else if (elapsed < 16213)
		speed = 83 * 10000 / 36;
	else if (elapsed < 16963)
		speed = 82 * 10000 / 36;
	else if (elapsed < 17463)
		speed = 81 * 10000 / 36;
	else if (elapsed < 18463)
		speed = 80 * 10000 / 36;
	else if (elapsed < 19463)
		speed = 79 * 10000 / 36;
	else if (elapsed < 20213)
		speed = 78 * 10000 / 36;
	else if (elapsed < 20963)
		speed = 77 * 10000 / 36;
	else if (elapsed < 22463)
		speed = 76 * 10000 / 36;
	else if (elapsed < 22963)
		speed = 75 * 10000 / 36;
	else if (elapsed < 23713)
		speed = 74 * 10000 / 36;
	else if (elapsed < 25213)
		speed = 73 * 10000 / 36;
	else if (elapsed < 26453)
		speed = 72 * 10000 / 36;
	else if (elapsed < 27203)
		speed = 71 * 10000 / 36;
	else if (elapsed < 27953)
		speed = 70 * 10000 / 36;
	else if (elapsed < 29193)
		speed = 69 * 10000 / 36;
	else if (elapsed < 29943)
		speed = 68 * 10000 / 36;
	else if (elapsed < 30693)
		speed = 67 * 10000 / 36;
	else if (elapsed < 32183)
		speed = 66 * 10000 / 36;
	else if (elapsed < 32933)
		speed = 65 * 10000 / 36;
	else if (elapsed < 33683)
		speed = 64 * 10000 / 36;
	else if (elapsed < 34683)
		speed = 63 * 10000 / 36;
	else if (elapsed < 35433)
		speed = 62 * 10000 / 36;
	else if (elapsed < 36433)
		speed = 61 * 10000 / 36;
	else if (elapsed < 37183)
		speed = 60 * 10000 / 36;
	else if (elapsed < 38433)
		speed = 59 * 10000 / 36;
	else if (elapsed < 39433)
		speed = 58 * 10000 / 36;
	else if (elapsed < 40183)
		speed = 57 * 10000 / 36;
	else if (elapsed < 40683)
		speed = 56 * 10000 / 36;
	else if (elapsed < 42183)
		speed = 55 * 10000 / 36;
	else if (elapsed < 43183)
		speed = 54 * 10000 / 36;
	else if (elapsed < 44683)
		speed = 53 * 10000 / 36;
	else if (elapsed < 45933)
		speed = 52 * 10000 / 36;
	else if (elapsed < 47183)
		speed = 51 * 10000 / 36;
	else if (elapsed < 48183)
		speed = 50 * 10000 / 36;
	else if (elapsed < 49433)
		speed = 49 * 10000 / 36;
	else
		speed = 48 * 10000 / 36;
...
static int
cmd_dyno_loss(UNUSED(int argc), UNUSED(char **argv)) {
	ecu_data *ep;
	...
	/* Check mass */
	if (dyno_get_mass() <= 0) {
		printf("The mass of the vehicle has not been set, please set the mass "
		       "first\n");
		return CMD_OK;
	}

	/* Show instructions */
	printf("To proceed loss determination, reach the maximum speed you will reach "
	       "during\n");
	printf("dyno, then push in the clutch, leaving the car in gear. Allow the car to "
	       "coast\n");
	...
	while (1) {
		...
		if (speed_previous == speed) { /* measure added: update display */
			...
			/* Display new measure */
			length = printf("%d (speed=%d km/h, d=%5.5f, f=%4.2f)\t ", nb,
					SPEED_ISO_TO_KMH(speed), dyno_loss_get_d(),
					dyno_loss_get_f());
 ...
 */
static int
cmd_dyno_setloss(int argc, char **argv) {
	if (argc > 1) {
		int assigned;
		double d;
...
static int
cmd_dyno_run(UNUSED(int argc), UNUSED(char **argv)) {
	ecu_data *ep;
	...
	/* Check mass */
	if (dyno_loss_done <= 0) {
		printf("The loss determination has not been done, please use command loss "
		       "or setloss first\n");
		return CMD_OK;
	}
...
static void
get_measures(dyno_measure **measures, int *nb_measures) {
	/* allocate memory */
...
static void
display_measures(dyno_measure *measures, int nb_measures) {
	int i;

	for (i = 0; i < nb_measures; i++) {
		printf("measure %d:\t%3.3f s. \tRPM: %d\t%3.3f m/s\t%3.2f km/h\n", (i + 1),
		       measures[i].millis / 1000.0, measures[i].rpm,
...
static int
cmd_dyno_measures(UNUSED(int argc), UNUSED(char **argv)) {
...
/* Display results */
static void
display_results(dyno_result *results, int nb) {
	int i;
	...
	for (i = 0; i < nb; i++) {
		printf("%d:\tRPM=%d\t\tpower=%d W (%d ch)\ttorque=%d Nm\n", i,
		       results[i].rpm, results[i].power, results[i].power_ch,
		       results[i].torque);
	...
	printf("\n");
	printf("Max power : %d ch (at %d RPM)\n", results[max_power_i].power_ch,
	       results[max_power_i].rpm);
	printf("Max torque : %d Nm (at %d RPM)\n", results[max_torque_i].torque,
	       results[max_torque_i].rpm);
	printf("\n");
}
...
/* Display graphs */
static void
display_graphs(dyno_result *results, int nb) {
	int row, col, step;
...
static int
cmd_dyno_result(UNUSED(int argc), UNUSED(char **argv)) {
	get_results();
...
static int
cmd_dyno_graph(UNUSED(int argc), UNUSED(char **argv)) {
	get_results();
 ...
 */
static int
cmd_dyno_save(int argc, char **argv) {
	char *filename;
	int rv;
