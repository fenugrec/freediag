...
set_init(void) {
	...
	global_cfg.speed = 10400; /* Comms speed; ECUs will probably send at 10416 bps
				     (96us per bit) */
	...
	global_cfg.L2idx = 0;
	global_cfg.L2proto = l2proto_list[0]->diag_l2_protocol; /* cannot guarantee 9141
								   was compiled...
...
const struct cmd_tbl_entry set_cmd_table[] = {
	{"help", "help [command]", "Gives help for a command", cmd_set_help, 0, NULL},
	{"?", "help [command]", "Gives help for a command", cmd_set_help, FLAG_HIDDEN,
	 NULL},

	{"interface", "interface [NAME]",
	 "Interface to use. Use set interface ? to get a list of names", cmd_set_interface,
	 0, NULL},

	{"display", "display [english/metric]", "English or metric display",
	 cmd_set_display, 0, NULL},

	{"speed", "speed [speed]", "ECU communications speed", cmd_set_speed, 0, NULL},
	{"testerid", "testerid [testerid]", "Source ID/address", cmd_set_testerid, 0,
	 NULL},
	{"destaddr", "destaddr [destaddr]", "Destination ID/address", cmd_set_destaddr, 0,
	 NULL},

	{"addrtype", "addrtype [func/phys]", "Address type, physical or functional.",
	 cmd_set_addrtype, 0, NULL},

	{"l1protocol", "l1protocol [protocolname]",
	 "Hardware (L1) protocol to use. Use 'set l1protocol ?' to show valid choices.",
	 cmd_set_l1protocol, 0, NULL},

	{"l2protocol", "l2protocol [protocolname]",
	 "Software (L2) protocol to use. Use 'set l2protocol ?' to show valid choices.",
	 cmd_set_l2protocol, 0, NULL},

	{"initmode", "initmode [modename]",
	 "Bus initialisation mode to use. Use 'set initmode ?' to show valid choices.",
...
const char *const l1_names[] = { // these MUST be in the same order as they are listed in
				 // diag_l1.h !!
	"ISO9141", "ISO14230", "J1850-VPW", "J1850-PWM", "CAN", "", "", "RAW", NULL};

// These MUST match the DIAG_L2_TYPE_* flags in diag_l2.h  so that
// l2_initmodes[DIAG_L2_TYPE_XX] == "XX" !!
const char *const l2_initmodes[] = {"5BAUD", "FAST", "CARB", NULL};
...
static int
cmd_set_custom(int argc, char **argv) {
	struct cfgi *cfgp;
	char *setstr;
	bool helping = 0;
	bool show_current = 0;
	...
	if (!global_dl0d) {
		...
		printf("No such item !\nAdditional items may be available after setting "
		       "the interface type.\nUse \"set interface NAME\" to set the "
		       "interface type.\n");
		return CMD_FAILED;
	}
...
cmd_set_show(UNUSED(int argc), UNUSED(char **argv)) {
	/* Show stuff; calling the cmd_set_*() functions with argc=0 displays the current
	 * setting. */
...
static int
cmd_set_interface(int argc, char **argv) {
	...
	if (strcmp(argv[1], "?") == 0) {
		helping = 1;
		printf("Valid interface names (not case sensitive): \n\t");
	}
...
static int
cmd_set_l2protocol(int argc, char **argv) {
	if (argc > 1) {
		int i, helping = 0, found = 0;
		if (strcmp(argv[1], "?") == 0) {
			helping = 1;
		...
		if (!found) {
			...
			printf("l2protocol: use \"set l2protocol ?\" to see list of "
			       "protocols\n");
		}
	} else {
...
static int
cmd_set_l1protocol(int argc, char **argv) {
	if (argc > 1) {
		int i, helping = 0, found = 0;
		if (strcmp(argv[1], "?") == 0) {
			helping = 1;
		...
		if (!found) {
			...
			printf("l1protocol: use \"set l1protocol ?\" to see list of "
			       "protocols\n");
		}
		return CMD_OK;
	}
...
static int
cmd_set_initmode(int argc, char **argv) {
	if (argc > 1) {
		int i, helping = 0, found = 0;
		if (strcmp(argv[1], "?") == 0) {
			helping = 1;
		...
		if (!found) {
			...
			printf("initmode: use \"set initmode ?\" to see list of "
			       "initmodes\n");
		}
		return CMD_OK;
	}
